...
# from pyarmor.examples.pybench.pybench import pickle
...
class NearFaultMotionSim():
	...
	def calc_impulse_response_filter(num_steps, input_filter, zeta, time_step_):
		...
		for i in prange(num_steps):
			...
			times = np.array([j * time_step_ for j in prange(num_steps - i)])
	...
	def simulate_white_noise(self,modulating_params,filter_params,num_steps,time_step_,t0):
		...
		###---计算单位脉冲响应
		# print("filterPara",filter_params)
	...
	def simulate_near_fault_ground_motion(self,pulse,parameters):
		...
		# print("apha_1:",alpha_1)
		# print("alpha_2:",alpha_2)
		# print("filter_params_1:",filter_params_1)
		# print("filter_params_2:",filter_params_2)
		###--确定模拟地震动长度
	...
	def runSimulation(self):
		...
		for i0 in trange(self.numSimMotions):
			if i0<self.numPulseMotion:##脉冲地震动模拟
				...
				pulseComponnetsList.append([pulse_comp1,pulse_comp2,returnPulseModelAcc,parameters_pulse[0,:5]])
			else:##非脉冲地震动模拟
				noPulse_comp1, noPulse_comp2,_ = self.simulate_near_fault_ground_motion(False, parameters_noPulse[int(i0-self.numPulseMotion), :])
...
def timeHistoryPlot(folderName,motionNumber=1):
	...
	filesDict = dict(Counter(f.split('_', 1)[0] for f in files))
	#####################################################################
	if filesDict[str(motionNumber)]==4: ###-脉冲地震动绘制
		...
		plt.savefig(f'general_ground motion {motionNumber} time history.jpg', dpi=600, bbox_inches='tight')
		plt.savefig(f'general_ground motion {motionNumber} time history.eps', dpi=600, bbox_inches='tight')
		plt.show()
	##############################################################
	elif filesDict[str(motionNumber)]==2: ###-非脉冲地震动绘制
		fig, axes = plt.subplots(3, 2, figsize=(20, 5))
		fig.suptitle(f'Ground motion {motionNumber} time history', fontsize=16)
		###---first column, main component
		###---acc(g)
		mainCompAcc = np.loadtxt(folderName + f"/{motionNumber}_mainComp.txt").tolist()
		timeHistory = [i1 * 0.005 for i1 in range(len(mainCompAcc))]
		axes[0, 0].plot(timeHistory, mainCompAcc, 'b', linewidth=1)
		axes[0, 0].set_ylabel('Acc(g)', fontsize=10)
		axes[0, 0].set_title('mainComp', fontsize=11)
		axes[0, 0].grid(True, which='both', linestyle='--', color='grey')
		axes[0, 0].set_xlim([0, timeHistory[-1]])
		absMax = max(np.abs(mainCompAcc)) * 1.1
		axes[0, 0].set_ylim([-absMax, absMax])
		###---vel(cm/s)
		mainCompVel = accToVelocity(dt, mainCompAcc)
		timeHistory = [i1 * 0.005 for i1 in range(len(mainCompVel))]
		axes[1, 0].plot(timeHistory, mainCompVel, 'b', linewidth=1)
		axes[1, 0].set_ylabel('Vel(cm/s)', fontsize=10)
		# axes[1, 0].set_title('PulseComp Acc', fontsize=11)
		axes[1, 0].grid(True, which='both', linestyle='--', color='grey')
		axes[1, 0].set_xlim([0, timeHistory[-1]])
		absMax = max(np.abs(mainCompVel)) * 1.1
		axes[1, 0].set_ylim([-absMax, absMax])
		###---disp(cm)
		mainCompDisp = velToDisplacement(dt,mainCompVel)
		timeHistory = [i1 * 0.005 for i1 in range(len(mainCompDisp))]
		axes[2, 0].plot(timeHistory, mainCompDisp, 'b', linewidth=1)
		axes[2, 0].set_ylabel('Disp(cm)', fontsize=10)
		axes[2, 0].set_xlabel('Time (s)', fontsize=10)
		# axes[2, 0].set_title('PulseComp Acc', fontsize=11)
		axes[2, 0].grid(True, which='both', linestyle='--', color='grey')
		axes[2, 0].set_xlim([0, timeHistory[-1]])
		absMax = max(np.abs(mainCompDisp)) * 1.1
		axes[2, 0].set_ylim([-absMax, absMax])
		...
		raise ValueError(f"motion number {motionNumber} not in the folder!")
########################################################################################################################
########################################################################################################################
def responseSpectraPlot(folderName,maxPeriod=10,beta=0.05,logCoord=True,SaLim=[0.01,10],SvLim=[0.01,500],SdLim=[0.001,500]):
	...
	from collections import Counter
	filesDict = dict(Counter(f.split('_', 1)[0] for f in files))
	motionNumbers=list(filesDict.keys())
	dt = 0.005  ###---time interval 0.005 s
	pulseComp_Sa=[]
	pulseComp_Sv=[]
	pulseComp_Sd=[]
	...
	pulseMode_Sv=[]
	pulseModel_Sd=[]
	for i1 in range(len(motionNumbers)):
		###---pulseComp acc(g)
		if filesDict[motionNumbers[i1]]==4:
			pulseCompAcc = np.loadtxt(folderName + f"/{motionNumbers[i1]}_pulseComp.txt")
		else:
			pulseCompAcc = np.loadtxt(folderName + f"/{motionNumbers[i1]}_mainComp.txt")
	...
	if logCoord:###-log-log coordinate
		####################################################################################
		###-ax1 set
		ax1.set_title('mainComp', fontsize=11)
		...
		ax1.set_xscale("log")
		ax1.set_yscale("log")
		for i2 in range(len(motionNumbers)):
			ax1.plot(TList, pulseComp_Sa[i2], "grey", linewidth=1,label=f'motion {motionNumbers[i2]}',picker=2)  ###-mainComp Sa(g)
		...
		ax3.set_ylabel('Sv (cm/s)', fontsize=10)
		ax3.grid(which="both")
		ax3.set_xlim(TList[0], maxPeriod)
		ax3.set_ylim(SvLim[0], SvLim[1])
		ax3.set_xscale("log")
		ax3.set_yscale("log")
		for i2 in range(len(motionNumbers)):
			ax3.plot(TList, pulseComp_Sv[i2], "grey", linewidth=1,label=f'motion {motionNumbers[i2]}',picker=2)  ###-mainComp Sa(g)
		ax3_mean = np.mean(pulseComp_Sv, axis=0)
		ax3_std = np.std(pulseComp_Sv, axis=0, ddof=0)  # ddof=1 为样本标准差；若要总体则 ddof=0
		...
		ax4.set_yscale("log")
		for i2 in range(len(motionNumbers)):
			ax4.plot(TList, orthogComp_Sv[i2], "grey", linewidth=1,label=f'motion {motionNumbers[i2]}',picker=2)  ###-pulseComp Sa(g)
		ax4_mean = np.mean(orthogComp_Sv, axis=0)
		ax4_std = np.std(orthogComp_Sv, axis=0, ddof=0)  # ddof=1 为样本标准差；若要总体则 ddof=0
		...
		ax5.set_ylabel('Sd (cm)', fontsize=10)
		ax5.set_xlabel('Period (s)', fontsize=10)
		ax5.grid(which="both")
		ax5.set_xlim(TList[0], maxPeriod)
		ax5.set_ylim(SdLim[0], SdLim[1])
		ax5.set_xscale("log")
		ax5.set_yscale("log")
		for i2 in range(len(motionNumbers)):
			ax5.plot(TList, pulseComp_Sd[i2], "grey", linewidth=1,label=f'motion {motionNumbers[i2]}',picker=2)  ###-pulseComp Sa(g)
		ax5_mean = np.mean(pulseComp_Sd, axis=0)
		ax5_std = np.std(pulseComp_Sd, axis=0, ddof=0)  # ddof=1 为样本标准差；若要总体则 ddof=0
		...
		####################################################################################
		###-ax6 set
		# 6.set_title('pulseModel', fontsize=11)
		ax6.set_ylabel('Sd (cm)', fontsize=10)
		ax6.set_xlabel('Period (s)', fontsize=10)
		ax6.grid(which="both")
		ax6.set_xlim(TList[0], maxPeriod)
		ax6.set_ylim(SdLim[0], SdLim[1])
		ax6.set_xscale("log")
		ax6.set_yscale("log")
		for i2 in range(len(motionNumbers)):
			ax6.plot(TList, orthogComp_Sd[i2], "grey", linewidth=1,label=f'motion {motionNumbers[i2]}',picker=2)  ###-pulseComp Sa(g)
		ax6_mean = np.mean(orthogComp_Sd, axis=0)
		ax6_std = np.std(orthogComp_Sd, axis=0, ddof=0)  # ddof=1 为样本标准差；若要总体则 ddof=0
		...
		ax3.set_ylim(SvLim[0], SvLim[1])
		for i2 in range(len(motionNumbers)):
			ax3.plot(TList, pulseComp_Sv[i2], "grey", linewidth=1,label=f'motion {motionNumbers[i2]}',picker=2)  ###-pulseComp Sa(g)
		ax3_mean = np.mean(pulseComp_Sv, axis=0)
		ax3_std = np.std(pulseComp_Sv, axis=0, ddof=0)  # ddof=1 为样本标准差；若要总体则 ddof=0
		ax3_upper = ax3_mean + ax3_std
		ax3_lower = ax3_mean - ax3_std
...
if __name__ == '__main__':
	...
	for i1 in range(10):
		print(f"第{i1+1}次模拟!")
		moment=np.random.uniform(6,7.5)
		Rrup=np.random.uniform(5,25)
		s_or_d=np.random.uniform(1.2,135)
		theta_or_phi=np.random.uniform(0,90)
		Vs30=np.random.uniform(400,1000)


		faultType="strikeSlip" ##Including: ["strikeSlip","reverseAndReverseOblique"]
		simulationType="onlyNoPulse"  ##Including: ["pulseAndNoPulse","onlyPulse","onlyNoPulse"]
		momentMag=moment
		zTOR=0
		Rrup=Rrup
		Vs30=Vs30
		...
		numSimMotions=1
		paraList.append([moment,Rrup,Vs30])
		...
		np.savetxt("pulseGroundMotions/"+str(i1+1)+".txt",np.asmatrix(pulseComponnetsList[0]).T,fmt="%.6f")
	np.savetxt("pulseGroundMotions/parameters.txt", paraList, fmt="%.2f %.2f %.2f")
	###############################################
	################---反应谱绘制
