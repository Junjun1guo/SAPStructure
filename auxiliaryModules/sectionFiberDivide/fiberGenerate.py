...
class CircleSection():
    ...
    def barMesh(self, outBarD, outBarDist, inBarD=None, inBarDist=None):
        ...
        barXListPlot = [reduce(operator.add, barXListPlot)]
        barYListPlot = [reduce(operator.add, barYListPlot)]
        return barFiberInfo,barXListPlot,barYListPlot

    def userBarMesh(self,userBarInfoList):
        """
        ...
        fiberXListReturn=[]
        fiberYListReturn=[]
        barFiberInfoListReturn=[]
        for each in userBarInfoList:
            outBarDistValue,barD,barDist=each[0],each[1],each[2]
            newR=self.outDiameter*0.5-outBarDistValue
            area = (np.pi * barD ** 2) / 4.0
            circumLength = 2 * np.pi * newR
            nBar = int(circumLength / barDist)
            angle = (2 * np.pi) / nBar
            fiberXList = [newR * np.cos(angle * i1) for i1 in range(1, nBar + 1)]
            fiberXListReturn=fiberXListReturn+fiberXList
            fiberYList = [newR * np.sin(angle * i2) for i2 in range(1, nBar + 1)]
            fiberYListReturn=fiberYListReturn+fiberYList
            barFiberInfo = [(xb, yb, area) for xb, yb in zip(fiberXList, fiberYList)]
            barFiberInfoListReturn=barFiberInfoListReturn+barFiberInfo
        barXListPlot=fiberXListReturn
        barYListPlot =fiberYListReturn
        barFiberInfo=barFiberInfoListReturn
        return barFiberInfo, barXListPlot, barYListPlot
