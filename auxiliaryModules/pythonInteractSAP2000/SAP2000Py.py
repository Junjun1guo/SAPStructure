...
class SAP2000Py():
    ...
    ----------------------------------------------------------------------------------------------------------------
    """
    def __init__(self,sapObject,sapModel):
    ...
    def define_section_PropFrame_GetSectProps(self,sectName):
        """
        ----------------------------------------------------------------------------------------------------------------
        ---This function retrieves properties for frame section---
        Inputs:
        sectName(str)-The name of an existing frame section property
        --------------------------------------------------------------
        Outputs:[Area,As2,As3,Torsion,I22,I33,S22,S33,Z22,Z33,R22,R33]
        ...
        ----------------------------------------------------------------------------------------------------------------
        """
        results=self.SapModel.PropFrame.GetSectProps(sectName)[1:]
        Area,AS2,AS3,Torsion,I22,I33,S22,S33,Z22,Z33,R22,R33=(results[0],results[1],results[2],results[3],
        results[4],results[5],results[6],results[7],results[8],results[9],results[10],results[11])
        return Area,Torsion,I22,I33
    ...
    def assign_LinkObj_GetLoadTargetForce(self,Name):
        ...
        T1,T2,T3,T4,T5,T6(float)-These are arrays of the relative distances along the link objects where the target
            ...
            T4 = relative location for T target force
            T5 = relative location for M2 target force
            T6 = relative location for M3 target
        ----------------------------------------------------------------------------------------------------------------
        """
