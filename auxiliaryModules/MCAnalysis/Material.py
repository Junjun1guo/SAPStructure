...
class Material():
	def __init__ (self, folderName,fiberSectName):
	...
	def barParameter(self, steelTag):
		...
		self.saveFold=os.getcwd()+f"\{self.folderName}\{self.sectName}"
		if not os.path.exists(self.saveFold):
			os.makedirs(self.saveFold)
		np.savetxt(self.saveFold+"/barParameter.txt", barPara, fmt="%0.6f")
		return barPara
	...
	def coverParameter(self, concreteTag):
		...
		np.savetxt(self.saveFold+ "/coverParameter.txt", coverPara, fmt="%0.6f")
		return coverPara
	...
	def coreParameterCircular(self, concreteTag, hoop, d, coverThick, roucc, s, ds, fyh):
		...
		np.savetxt(self.saveFold+ "/coreParameter.txt", corePara, fmt="%0.6f")
		return corePara
