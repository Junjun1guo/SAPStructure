...
class MC():
	def __init__(self, folderName,fiberFolderName,fiberSectName, direction):
		"""
		:param sectName: section name
		:param direction: calculated direction ('X' or 'Y')
		"""
		self.savedFolder=folderName
		self.savedFiberFolder=fiberFolderName
		self.sectName = fiberSectName
	...
	def MCAnalysis(self, axialLoad, moment, maxMu=30, numIncr=100):
		...
		coverParameter = np.loadtxt(self.savedFolder+"/"+self.sectName+"/coverParameter.txt")
		uniaxialMaterial('Concrete04', 1, coverParameter[0], coverParameter[1], coverParameter[2], coverParameter[3])
		coreParameter = np.loadtxt(self.savedFolder+"/"+self.sectName+"/coreParameter.txt")
		uniaxialMaterial('Concrete04', 2, coreParameter[0], coreParameter[1], coreParameter[2], coreParameter[3])
		barParameter = np.loadtxt(self.savedFolder+"/"+self.sectName+"/barParameter.txt")
		...
		coverfibers = np.loadtxt(self.savedFiberFolder+"/"+self.sectName+"_coverFiber.txt")
		for coverfiber in coverfibers:
			fiber(coverfiber[0], coverfiber[1], coverfiber[2], 1)
		corefibers = np.loadtxt(self.savedFiberFolder+"/"+self.sectName+"_coreFiber.txt")
		for corefiber in corefibers:
			fiber(corefiber[0], corefiber[1], corefiber[2], 2)
		barfibers = np.loadtxt(self.savedFiberFolder+"/"+self.sectName+"_barFiber.txt")
		for barfiber in barfibers:
			fiber(barfiber[0], barfiber[1], barfiber[2], 3)

		element('zeroLengthSection', 1, 1, 2, 1, '-oirent', 1, 0, 0, 0, 1, 0)

		if os.path.exists(self.savedFolder+"/"+self.sectName+'/coreRecorder'):
			shutil.rmtree(self.savedFolder+"/"+self.sectName+'/coreRecorder')
		if os.path.exists(self.savedFolder+"/"+self.sectName+'/barRecorder'):
			shutil.rmtree(self.savedFolder+"/"+self.sectName+'/barRecorder')
		os.makedirs(self.savedFolder+"/"+self.sectName+'/coreRecorder')
		os.makedirs(self.savedFolder+"/"+self.sectName+'/barRecorder')

		setMaxOpenFiles(5000)
		recorder('Node','-file',self.savedFolder+"/"+self.sectName+'/MomentCurvature.txt','-time',
				 '-node',2,'-dof',6-flagy,'disp')
		for i in range(len(corefibers)):
			recorder('Element','-file',self.savedFolder+"/"+self.sectName+'/coreRecorder/'+str(i+1)+'.txt','-time',
					 '-ele',1,'section','fiber',str(corefibers[i,0]),str(corefibers[i,1]),'stressStrain')
		for j in range(len(barfibers)):
			recorder('Element','-file',self.savedFolder+"/"+self.sectName+'/barRecorder/'+str(j+1)+'.txt','-time',
					 '-ele',1,'section','fiber',str(barfibers[j,0]),str(barfibers[j,1]),'stressStrain')
		...
		if self.direction == 'X':
			ky = np.loadtxt(self.savedFolder+"/"+self.sectName+"/yieldCurvature.txt")[0]
		else:
			ky = np.loadtxt(self.savedFolder+"/"+self.sectName + "/yieldCurvature.txt")[1]
		maxK = ky*maxMu
		dK = maxK / numIncr
	...
	def plotLinearRegre (self,x1,y1,x2,y2,xyield,yyield,xmax,ymax):
		...
		if not os.path.exists(self.savedFolder+"/"+self.sectName+'/MCFig'):
			os.makedirs(self.savedFolder+"/"+self.sectName+'/MCFig')
		...
		plt.savefig(self.savedFolder+"/"+self.sectName+'/MCFig/'+self.sectName+'-'+self.direction+'-'+str(int(yliner[2]))+".png",
					dpi = 600)
		plt.savefig(self.savedFolder+"/"+self.sectName+'/MCFig/' + self.sectName + '-' + self.direction + '-' + str(int(yliner[2])) + ".eps")
		plt.show()

	def MCCurve(self):

		fsy = np.loadtxt(self.savedFolder+"/"+self.sectName + "/barParameter.txt")[0]
		Es = np.loadtxt(self.savedFolder+"/"+self.sectName + "/barParameter.txt")[2]
		ey = fsy/Es
		esu = np.loadtxt(self.savedFolder+"/"+self.sectName + "/barParameter.txt")[5]
		ecu = np.loadtxt(self.savedFolder+"/"+self.sectName + "/coreParameter.txt")[2]

		try:
			barDir =  os.listdir(self.savedFolder+"/"+self.sectName+'/barRecorder/')
			coreDir =  os.listdir(self.savedFolder+"/"+self.sectName+'/coreRecorder/')
		except:
			print("Please run MC Analysis")

		momentCurvature=np.loadtxt(self.savedFolder+"/"+self.sectName+"/MomentCurvature.txt")
		...
		for eachFilePath in barDir:
			barStressStrain=np.loadtxt(self.savedFolder+"/"+self.sectName+'/barRecorder/'+eachFilePath)
		...
		for eachFile1 in coreDir:
			coreStressStrain=np.loadtxt(self.savedFolder+"/"+self.sectName+'/coreRecorder/'+eachFile1)
